...
async function importTargetedEvents() {
  try {
    console.log("Importing targeted events...");
    
    // Read targeted events
    const scrapedEventsPath = path.join(__dirname, "../attached_assets/targeted_scraped_events.json");
    ...
    console.error("Error importing targeted events:", error);
  }
}

importTargetedEvents().then(() => {
