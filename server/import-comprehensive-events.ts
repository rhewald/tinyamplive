...
async function importComprehensiveEvents() {
  try {
    console.log("Importing comprehensive events...");
    
    // Read comprehensive events
    const scrapedEventsPath = path.join(__dirname, "../attached_assets/comprehensive_venue_scraped_events.json");
    ...
    for (const scrapedEvent of scrapedEvents) {
      try {
        ...
        // Very aggressive filtering for bad artist names
        const validArtistNames = artistNames.filter(name => 
          ...
          !name.includes('indie') &&
          !name.includes('emo') &&
          !name.includes('shoegaze') &&
          !name.includes('post') &&
          !name.includes('metal') &&
          !name.includes('psychedelic') &&
          !name.includes('garage') &&
          !name.includes('alternative') &&
          !name.includes('americana') &&
          !name.includes('country') &&
          !name.includes('jazz') &&
          !name.includes('swing') &&
          !name.includes('blues') &&
          !name.includes('stoner') &&
          !name.includes('synthwave') &&
          !name.includes('Mon') &&
          !name.includes('Tue') &&
          !name.includes('Wed') &&
          !name.includes('Thu') &&
        ...
        const eventSlug = scrapedEvent.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').substring(0, 50) + '-' + Date.now();
    ...
    console.error("Error importing comprehensive events:", error);
  }
}

importComprehensiveEvents().then(() => {
