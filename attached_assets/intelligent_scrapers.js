...
class IntelligentEventScraper {
  ...
  async scrapeBottomOfHillIntelligent() {
    try {
      console.log('Scraping Bottom of the Hill with intelligent filtering...');
  ...
  async scrapeIndependentIntelligent() {
    try {
      console.log('Scraping The Independent with intelligent filtering...');
  ...
  async scrapeCafeDuNordIntelligent() {
    try {
      console.log('Scraping Café du Nord with intelligent filtering...');
  ...
  async scrapeAllVenuesIntelligent() {
    try {
      await this.init();
      
      const allEvents = [];
      
      // Scrape each venue with intelligent approach
      const bthEvents = await this.scrapeBottomOfHillIntelligent();
      allEvents.push(...bthEvents);
      
      await this.page.waitForTimeout(2000);
      
      const independentEvents = await this.scrapeIndependentIntelligent();
      allEvents.push(...independentEvents);
      
      await this.page.waitForTimeout(2000);
      
      const cafeEvents = await this.scrapeCafeDuNordIntelligent();
      ...
      // Save to file
      const outputPath = path.join(__dirname, 'intelligent_scraped_events.json');
...
async function main() {
  const scraper = new IntelligentEventScraper();
  const events = await scraper.scrapeAllVenuesIntelligent();
